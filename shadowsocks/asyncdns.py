...
class DNSResolver(object):
		...
		if response and response.hostname:
			...
			for answer in response.answers:
				if answer[1] in (QTYPE_A, QTYPE_AAAA) and  answer[2] == QCLASS_IN:
					ip = answer[0]
					break
			if not ip and self._hostname_status.get(hostname, STATUS_IPV6)	== STATUS_IPV4:
	...
	def _send_req(self, hostname, qtype):
		...
		for server in self._servers:
			logging.warn('resolving %s with type %d using server %s' % (hostname, qtype, server))
# 			common.error_to_file('resolving %s with type %d using server %s' % (hostname, qtype, server),self._config)
